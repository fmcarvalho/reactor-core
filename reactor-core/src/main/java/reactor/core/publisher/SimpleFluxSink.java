 ...
 * @author Simon Baslé
 */
public interface SimpleFluxSink<T> extends FluxSink<T> {

	/**
	 * Converts the {@link SimpleFluxSink} to a {@link Flux}, allowing to compose
	 * operators on it.
	 * <p>
	 ...
	 *
	 * @return the {@link SimpleFluxSink} viewed as a {@link Flux}
	 */
	Flux<T> toFlux();

	/**
	 * Converts the {@link SimpleFluxSink} to a {@link Mono}, allowing to compose
	 * operators on it.
	 *
	 * @return the {@link SimpleFluxSink} viewed as a {@link Mono}
	 */
	Mono<T> toMono();
	...
	//=== Direct
	/**
	 * Create a new direct {@link SimpleFluxSink}
	 *
	 * @param <O> Type of data signals
	 * @return a direct {@link SimpleFluxSink}
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> direct() {
	...
	/**
	 * Create a new unicast {@link SimpleFluxSink} that will buffer on an internal
	 * queue in an unbounded fashion.
	 *
	 * @param <O> the data type
	 * @return a unicast {@link SimpleFluxSink}
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> unicast() {
		return new FluxProcessor.SinkFacade<>(UnicastProcessor.create());
	}

	/**
	 * Create a new unicast {@link SimpleFluxSink} that will buffer on a provided
	 * queue in an unbounded fashion.
	 *
	 ...
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> unicast(Queue<O> queue) {
	 ...
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> unicast(Queue<O> queue, Disposable endCallback) {
	...
	/**
	 * Create a {@link SimpleFluxSink} that caches the last element it has pushed,
	 ...
	 * @param <O> the data type
	 *
	 * @return a new {@link SimpleFluxSink} that replays its last pushed element to each new
	 * {@link Subscriber}
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> replayLast() {
	 ...
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> replayLastOrDefault(@Nullable O value) {
		return new FluxProcessor.SinkFacade<>(ReplayProcessor.cacheLastOrDefault(value));
	}

	/**
	 * Create a new {@link SimpleFluxSink} that replays an unbounded number of elements,
	 * using a default internal {@link Queues#SMALL_BUFFER_SIZE Queue}.
	 *
	 * @param <O> the data type
	 *
	 * @return a new {@link SimpleFluxSink} that replays the whole history to each new
	 * {@link Subscriber}.
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> replayAll() {
		return new FluxProcessor.SinkFacade<>(ReplayProcessor.create());
	}

	/**
	 * Create a new {@link SimpleFluxSink} that replays up to {@code historySize}
	 * elements.
	 *
	 * @param historySize the backlog size, ie. maximum items retained for replay.
	 * @param <O> the data type
	 *
	 * @return a new {@link SimpleFluxSink} that replays a limited history to each new
	 * {@link Subscriber}.
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> replaySize(int historySize) {
		return new FluxProcessor.SinkFacade<>(ReplayProcessor.create(historySize, false));
	}

	/**
	 * Create a new {@link SimpleFluxSink} that either replay all the elements or a
	 ...
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> replaySize(int historySize, boolean unbounded) {
		return new FluxProcessor.SinkFacade<>(ReplayProcessor.create(historySize, unbounded));
	}

	/**
	 * Creates a time-bounded replay {@link SimpleFluxSink}.
	 * <p>
	 * In this setting, the {@link SimpleFluxSink} internally tags each observed item
	 ...
	 * <p>
	 * If an subscriber subscribes while the {@link SimpleFluxSink} is active, it will
	 ...
	 * at T=11, it will find an empty {@link SimpleFluxSink} with just an {@code
	 * onCompleted} signal.
	 *
	 * @param <O> the type of items observed and emitted by the Processor
	 * @param maxAge the maximum age of the contained items
	 *
	 * @return a new {@link SimpleFluxSink} that replays elements based on their age.
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> replayTimeout(Duration maxAge) {
	 ...
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> replayTimeout(Duration maxAge, Scheduler scheduler) {
		return new FluxProcessor.SinkFacade<>(ReplayProcessor.createTimeout(maxAge, scheduler));
	}

	/**
	 * Creates a time- and size-bounded replay {@link SimpleFluxSink}.
	 * <p>
	 * In this setting, the {@link SimpleFluxSink} internally tags each received item
	 ...
	 * <p>
	 * When subscribers subscribe to a terminated {@link SimpleFluxSink}, they observe
	 ...
	 *
	 * @return a new {@link SimpleFluxSink} that replay up to {@code size} elements, but
	 * will evict them from its history based on their age.
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> replaySizeAndTimeout(int size, Duration maxAge) {
	 ...
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> replaySizeAndTimeout(int size,
			Duration maxAge,
			Scheduler scheduler) {
	...
	/**
	 * Create a new emitter {@link SimpleFluxSink} using {@link Queues#SMALL_BUFFER_SIZE}
	 * backlog size and auto-cancel.
	 *
	 * @param <O> the data type
	 *
	 * @return an emitter {@link SimpleFluxSink}
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> emitter() {
	 ...
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> emitter(boolean autoCancel) {
		return new FluxProcessor.SinkFacade<>(EmitterProcessor.create(autoCancel));
	}

	/**
	 * Create a new  emitter {@link SimpleFluxSink} using the provided backlog size, with auto-cancel.
	 *
	 * @param <O> the data type
	 ...
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> emitter(int bufferSize) {
		return new FluxProcessor.SinkFacade<>(EmitterProcessor.create(bufferSize));
	}

	/**
	 * Create a new  emitter {@link SimpleFluxSink} using the provided backlog size and auto-cancellation.
	 *
	 * @param <O> the data type
	 * @param bufferSize the internal buffer size to hold signals
	 * @param autoCancel automatically cancel
	 ...
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> emitter(int bufferSize, boolean autoCancel) {
	...
	/**
	 * Create a new <strong>shared</strong> async {@link SimpleFluxSink} using
	 ...
	 *
	 * @param <O> the data type
	 * @return a new shared async {@link SimpleFluxSink}
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> async() {
	...
	/**
	 * Create a new async <strong>shared</strong> {@link SimpleFluxSink} using the
	 ...
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> async(String name, int bufferSize) {
		return new FluxProcessor.SinkFacade<>(TopicProcessor.<O>builder()
				.share(true)
				.name(name)
	...
	/**
	 * Create a new <strong>unshared</strong> async {@link SimpleFluxSink} using the
	 ...
	 */
	@SuppressWarnings("deprecation")
	static <O> SimpleFluxSink<O> asyncUnshared(String name, int bufferSize) {
		return new FluxProcessor.SinkFacade<>(TopicProcessor.<O>builder()
				.share(false)
				.name(name)
				.bufferSize(bufferSize)
